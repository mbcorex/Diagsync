...
import { mergeRadiologyPerTestSections, parseRadiologyPerTestSections, type RadiologyPerTestSection } from "./radiology-report-sections";
...
export async function getRadiologyTasks(
  ...
  const distinctTestIds = Array.from(new Set(orders.map((order) => order.testId)));

  const lastReports = distinctTestIds.length

    ? await prisma.radiologyTestLastReport.findMany({

        where: { organizationId: actor.organizationId, testId: { in: distinctTestIds } },
  ...
  const lastReportByTestId = new Map(lastReports.map((report) => [report.testId, report]));
  ...
  const tasksWithImageCount = rows.map((task) => {
    ...
    const testOrders = task.testOrderIds
      ...
      .map((order) => ({
        ...
        test: {
          ...
          lastReport: lastReportByTestId.get(order.testId) ?? null,
...
export async function submitRadiologyTask(
  ...
  const parsedPerTestSections = parseRadiologyPerTestSections(reportExtraFields);
  ...
  if (!hasRequiredReportFields({ ...task.radiologyReport, extraFields: reportExtraFields }, task.testOrderIds)) {
    ...
    const perTestByOrderId = new Map(parsedPerTestSections.map((row) => [row.testOrderId, row]));

    for (const id of task.testOrderIds) {

      const row = perTestByOrderId.get(id);
  ...
  try {
    const orderTestIds = await prisma.testOrder.findMany({
      where: { id: { in: task.testOrderIds }, organizationId: actor.organizationId },
      select: { id: true, testId: true },
    });
    const perTestByOrderId = new Map(parsedPerTestSections.map((row) => [row.testOrderId, row]));
    const defaultFindings = task.radiologyReport.findings?.trim() ?? "";
    const defaultImpression = task.radiologyReport.impression?.trim() ?? "";
    const defaultNotes = task.radiologyReport.notes?.trim() ?? "";
    for (const order of orderTestIds) {
      const section = perTestByOrderId.get(order.id);
      const findings = (section?.findings ?? defaultFindings).trim();
      const impression = (section?.impression ?? defaultImpression).trim();
      const notes = (section?.notes ?? defaultNotes).trim();
      if (!findings && !impression) continue;
      await prisma.radiologyTestLastReport.upsert({
        where: { organizationId_testId: { organizationId: actor.organizationId, testId: order.testId } },
        create: {
          organizationId: actor.organizationId,
          testId: order.testId,
          findings,
    ...
    console.error("[RAD_TASK_LAST_REPORT_SYNC]", error);
  }
  await createAuditLog({
