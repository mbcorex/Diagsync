...
type Task = {
  ...
  testOrders: Array<{
    ...
    test: {
      ...
      lastReport?: { findings: string; impression: string; notes?: string | null } | null;
    };
  }>;
};
...
export function RadiologyTaskBoard() {
    ...
    for (const task of rows) {
      ...
      for (const order of task.testOrders) {
        ...
        testReports[order.id] = {
          findings: perTest?.findings ?? task.radiologyReport?.findings ?? order.test.lastReport?.findings ?? "",
          impression: perTest?.impression ?? task.radiologyReport?.impression ?? order.test.lastReport?.impression ?? "",
  ...
  async function persistReportDraft(taskId: string, draft: Draft = draftsRef.current[taskId] ?? EMPTY_DRAFT) {
    const task = tasksRef.current.find((row) => row.id === taskId);
    if (task && !task.canEdit) {
      throw new Error("FORBIDDEN_TASK");
    }

    const testReports: RadiologyPerTestSection[] = Object.entries(draft.testReports ?? {}).map(([testOrderId, value]) => ({
      testOrderId,
    ...
    if (!json.success) {
      throw new Error(json.error ?? "Unable to save report");
    }
  ...
  async function saveReport(taskId: string) {
    ...
    try {
      const draft = drafts[taskId] ?? EMPTY_DRAFT;
      if (tasks.find((row) => row.id === taskId) && !tasks.find((row) => row.id === taskId)?.canEdit) {
        setError("This task is assigned to another radiographer. Please click Start on your own pending task.");
        return;
      }
      await persistReportDraft(taskId, draft);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void persistReportDraft(task.id, draftsRef.current[task.id] ?? EMPTY_DRAFT).catch(() => undefined);
    }, 700);
