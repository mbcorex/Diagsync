...
export function ReportWorkspace({ role }: { role: "MD" | "HRM" | "SUPER_ADMIN" | "RECEPTIONIST" }) {
  ...
  const [printLetterheadMode, setPrintLetterheadMode] = useState<"uploaded" | "custom" | "without">("uploaded");
  ...
  function previewUrl(
    reportId: string,
    letterheadMode: "uploaded" | "custom" | "without",
    opts?: { showPrintButton?: boolean; autoPrint?: boolean }
  ) {
    const query = new URLSearchParams();
    if (letterheadMode === "without") query.set("letterhead", "without");
    else if (letterheadMode === "custom") query.set("letterhead", "custom");
    else query.set("letterhead", "uploaded");
  ...
  function pdfUrl(reportId: string, letterheadMode: "uploaded" | "custom" | "without") {
    const query = new URLSearchParams();
    if (letterheadMode === "without") query.set("letterhead", "without");
    else if (letterheadMode === "custom") query.set("letterhead", "custom");
    else query.set("letterhead", "uploaded");
              ...
              {(canHrmRelease || canReceptionDispatch) && (
                <div className="p-4 space-y-3">
                  ...
                  <div className="rounded border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] text-slate-600">
                    Print option: choose uploaded, custom autogenerated, or without letterhead. WhatsApp always uses the uploaded letterhead.
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => { setPreviewLoaded(false); setPrintLetterheadMode(e.target.value as "uploaded" | "custom" | "without"); }}
